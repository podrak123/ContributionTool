...
namespace Git.hub
{
    ...
    public class Client
    {
        ...
        /// </summary>
        /// <returns></returns>
        /// <exception cref="System.Exception">Bad Credentials</exception>
        private IList<User> getExistingUsers()
        {
            string usersFilePath = ConfigurationManager.AppSettings["usersfilepath"];
            string usersText = File.ReadAllText(usersFilePath);
        ...
        /// </summary>
        /// <exception cref="System.Exception">Bad Credentials</exception>
        public IList<User> getNewUsers()
        {
            string usersFilePath = ConfigurationManager.AppSettings["usersfilepath"];

            // get the existing list
            var users = this.getExistingUsers();

            int nextId = users.Count > 0 ? users.Last().Id : 0;

            // fetch the new ones
            var request = new RestRequest("/users?since=" + nextId);

            var newUsers = client.GetList<User>(request);
            if (newUsers == null)
                throw new Exception("Bad Credentials");

            ((List<User>)users).AddRange(newUsers);
            string usersText = JsonConvert.SerializeObject(users);
            File.WriteAllText(usersFilePath, usersText);
        ...
        {
            var request = new RestRequest("/users/{name}");
            request.AddUrlSegment("name", login);

            var user = client.Get<User>(request).Data;
            if (user == null)
                return null;
